...
{
  ascon_hash_ctx_t* ctx = malloc(sizeof(ascon_hash_ctx_t));
  return ctx;
}
                     ...
                     size_t digest_len)
{
    ...
    ascon_hash_ctx_t temp_ctx;
    memcpy(&temp_ctx, ctx, sizeof(ascon_hash_ctx_t));
    hash_final(ascon_permutation_12, &temp_ctx, digest, digest_len);
}
                      ...
                      size_t digest_len)
{
    ...
    hash_final(ascon_permutation_8, &temp_ctx, digest, digest_len);
}
...
ASCON_API void
ascon_hash_free(ascon_hash_ctx_t* const ctx)
{
    free(ctx);
}
