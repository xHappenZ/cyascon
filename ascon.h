...
typedef struct ascon_bufstate
{
    ...
    /** Variable to distinguish between Ascon variants.*/
    uint8_t variant;
...
/**
 * Cipher context creation for Ascon hashing streaming mode. (Cython)
 *
 * Allocates memory on the heap for the cipher-context struct and
 * returns a pointer to it. This is necessary for the Cython implementation
 * of the streaming mode, as the state is required to be a C-type struct for
 * further operation in C, but is stored externally in a Cython wrapper.
 */
ascon_hash_ctx_t* new_hash_ctx();

/**
 * Cipher context creation for Ascon AEAD. (Cython - streaming mode)
 *
 * Allocates memory on the heap for the cipher-context struct and
 * returns a pointer to it. This is necessary for the Cython implementation
 * of the streaming mode, as the state is required to be a C-type struct for
...
/**
 * Offline symmetric encryption using Ascon128. (Cython - complete)
 *
 ...
 *
 * @warning
 * This function was modified in order to be used in the Cython implementations. The
 * only difference is that it creates a copy of the cipher context on the stack and
 * clears the memory before it returns. The state that is stored in the Cython wrapper
 * remains unaltered. This function may only be called with a clear state after initialization.
 * 
 * @warning
 ...
 */
ASCON_API void
ascon_aead128_encrypt(ascon_aead_ctx_t* const ctx,
...
/**
 * Online symmetric encryption using Ascon128, finalisation and tag generation. (Cython-intermediate)
 *
 * Finalises the authenticated encryption for a disposable copy of the state by returning any remaining buffered
 * ciphertext and the authentication tag. The total ciphertext length is
 * equal to the total plaintext length.
 *
 * It will securely erase the content of a disposable copy of the \p ctx struct before returning.
 *
 * The calling order for encryption with intermediate tags is:
 * 1. ascon_aead128_init() - once only
 * 2. ascon_aead128_assoc_data_update() - 0 or more times
 * 3. ascon_aead128_encrypt_update() - 0 or more times, see warning
 * 4. ascon_aead128_encrypt_intermediate() - once only (generates the same ciphertext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 *
 * @warning
 * Using the AEAD to just authenticate any associated data with no
 * plaintext to be encrypted is not recommended as the AEAD algorithm is not
 ...
 *
 * @warning
 * This function has no effect on the state, it only generates the last ciphertext block and the tag which authenticates
 * the ciphertext generated to this point. After calling this function, it's possible to use the update() function again
 * as only the disposable copy of the state was finalized.
 * 
 * @warning
 * A copy of the secret key is kept in the \p ctx struct and securely erased
 * during this function call. In case the encryption session is interrupted
 * and never finalised (this function is never called), clear the context with
 * ascon_aead_cleanup() to erase the key copy. (only for the copy of ctx)
 *
 * @param[in, out] ctx the encryption context, handling the cipher
 *       state and buffering of incoming data to be processed. It will not
 *       be changed by this function. Not NULL.
 * @param[out] ciphertext trailing encrypted data still available in the
 *       buffer of the buffered updating. This function will write
 *       [0, #ASCON_RATE[ bytes into \p ciphertext.
 *       The exact number of written bytes is indicated by the return value.
...
/**
 * Online symmetric decryption using Ascon128, finalisation and generation of the expected tag. (Cython-intermediate)
 *
 * Finalises the authenticated decryption disposable copy of the state by returning any remaining buffered
 * plaintext and the valid tag for the given ciphertext.
 * The total plaintext length is equal to the total ciphertext length.
 *
 * It will securely erase the content of a disposable copy of the \p ctx struct before returning.
 *
 ...
 * 2. ascon_aead128_assoc_data_update() - 0 or more times
 * 3. ascon_aead128_decrypt_update() - 0 or more times, see warning
 * 4. ascon_aead128_decrypt_intermediate() - once only (generates the same plaintext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 * 
 * @warning
 * This function has no effect on the state, it only generates the last plaintext block and verifies the tag which authenticates
 * the ciphertext generated to this point. After calling this function, it's possible to use the update() function again
 * as only the disposable copy of the state was finalized. It is not recommended to use this function without a wrapper
 * as instead of verifying the validity of a given tag, it computes the tag. This is required for the Cython implementation
 * which implements intermediate tags.
 *
 * @warning
 * A copy of the secret key is kept in the \p ctx struct and securely erased
 * during this function call. In case the decryption session is interrupted
 ...
 *       fingerprint), which is to be used as the expected tag in a verify function.
 * @param[in] tag_len length of the \p tag to generate in bytes. It's required to be
 *       the same length as generated during the encryption.
 * @returns number of bytes written into \p plaintext. The value is in the
...
/**
 * Freeing of the allocated memory for the cipher context.
 *
 * This function is called when the Cython object that handles
 * the AEAD is deleted.
 *
 *
 * @param[in, out] ctx pointer to the memory to be freed.
 */
ASCON_API void
ascon_aead_free(ascon_aead_ctx_t* const ctx);

/**
 * Freeing of the allocated memory for the cipher context for
 * Ascon hashing.
 *
 * This function is called when the Cython object that handles
 * the hashing is deleted.
 *
 *
 * @param[in, out] ctx pointer to the memory to be freed.
 */
...
/**
 * Offline symmetric encryption using Ascon128a, which uses a double data rate (Ctyhon - complete)
...
*/
ASCON_API void
ascon_aead128a_encrypt(ascon_aead_ctx_t* const ctx,
                      const uint8_t* assoc_data,
                      const uint8_t* plaintext,
                      size_t assoc_data_len,
                      size_t plaintext_len,
...
/**
 * Online symmetric encryption using Ascon128a, finalisation and tag generation. (Cython-intermediate)
 *
 * Finalises the authenticated encryption for a disposable copy of the state by returning any remaining buffered
 * ciphertext and the authentication tag. The total ciphertext length is
 * equal to the total plaintext length.
 ...
 * 2. ascon_aead128a_assoc_data_update() - 0 or more times
 * 3. ascon_aead128a_encrypt_update() - 0 or more times, see warning
 * 4. ascon_aead128a_encrypt_intermediate() - once only (generates the same ciphertext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 *
 * @warning
 * Using the AEAD to just authenticate any associated data with no
...
/**
 * Online symmetric decryption using Ascon128a, finalisation and generation of the expected tag. (Cython-intermediate)
 *
 * Finalises the authenticated decryption disposable copy of the state by returning any remaining buffered
 * plaintext and the valid tag for the given ciphertext.
 * The total plaintext length is equal to the total ciphertext length.
 ...
 * 2. ascon_aead128a_assoc_data_update() - 0 or more times
 * 3. ascon_aead128a_decrypt_update() - 0 or more times, see warning
 * 4. ascon_aead128a_decrypt_intermediate() - once only (generates the same plaintext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 * 
 * @warning
 * This function has no effect on the state, it only generates the last plaintext block and verifies the tag which authenticates
 ...
 * @param[in] expected_tag Message Authentication Code (MAC, a.k.a. cryptographic tag,
 *       fingerprint), which is to be used as the expected tag in a verify function.
 * @param[in] expected_tag_len length of the \p tag to generate in bytes. It's required to be
 *       the same length as generated during the encryption.
 * @returns number of bytes written into \p plaintext. The value is in the
 *        interval [0, #ASCON_DOUBLE_RATE[, i.e. whatever remained in the buffer
 *        after the last update call.
...
/**
 * Offline symmetric encryption using Ascon80pq, which uses a larger key. (Cython - complete)
 *
 * Encrypts the data which is already available as a whole in a contiguous
 * buffer, authenticating any optional associated data in the process.
 * Provides the ciphertext and the authentication tag as output.
...
*/
ASCON_API void
ascon_aead80pq_encrypt(ascon_aead_ctx_t* const ctx,
                      const uint8_t* assoc_data,
                      const uint8_t* plaintext,
                      size_t assoc_data_len,
                      size_t plaintext_len,
...
/**
 * Online symmetric encryption using Ascon80pq, finalisation and tag generation. (Cython-intermediate)
 *
 * Finalises the authenticated encryption for a disposable copy of the state by returning any remaining buffered
 * ciphertext and the authentication tag. The total ciphertext length is
 * equal to the total plaintext length.
 ...
 * 2. ascon_aead80pq_assoc_data_update() - 0 or more times
 * 3. ascon_aead80pq_encrypt_update() - 0 or more times, see warning
 * 4. ascon_aead80pq_encrypt_intermediate() - once only (generates the same ciphertext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 *
 * @warning
 * Using the AEAD to just authenticate any associated data with no
...
/**
 * Online symmetric decryption using Ascon80pq, finalisation and generation of the expected tag. (Cython-intermediate)
 *
 * Finalises the authenticated decryption disposable copy of the state by returning any remaining buffered
 * plaintext and the valid tag for the given ciphertext.
 * The total plaintext length is equal to the total ciphertext length.
 ...
 * 2. ascon_aead80pq_assoc_data_update() - 0 or more times
 * 3. ascon_aead80pq_decrypt_update() - 0 or more times, see warning
 * 4. ascon_aead80pq_decrypt_intermediate() - once only (generates the same plaintext + tag on second execution without update)
 * 5. repeat step 3 & 4 - 0 or more times, see warning
 * 
 * @warning
 * This function has no effect on the state, it only generates the last plaintext block and verifies the tag which authenticates
...
/**
 * Online Ascon-Hasha with custom digest length (eXtendable Output Function,
 * XOF), finalisation and digest generation. (Cython-intermediate)
 *
 * Finalises the hashing by returning the digest of the message. This
 * function doesn't apply any changes to the state. Instead it makes a
 * disposable copy on the stack to which all chages are applied. The
 * memory of the copy will be cleared before returning from this funcion.
 *
 * Uses 8-round `Pb` permutations during absorption and squeezing, lighter
 * version of the Ascon-Hash function.
 *
 ...
 *
 * @param[in, out] ctx the hashing context, handling the hash function state
 *       and buffering of incoming data to be processed. All changes are applied
 *       to a copy. Not NULL.
 * @param[out] digest fingerprint of the message, output of the hash function,
 *       of \p digest_size bytes.
 * @param[in] digest_len desired length of the \p digest in bytes.
...
/**
 * Online Ascon-Hash with custom digest length (eXtendable Output Function,
 * XOF), finalisation and digest validation of the expected one. (Cython-intermediate)
 *
 * Uses 12-round `Pb` permutations during absorption and squeezing.This
 * function doesn't apply any changes to the state. Instead it makes a
 * disposable copy on the stack to which all chages are applied. The
 * memory of the copy will be cleared before returning from this funcion.
 *
