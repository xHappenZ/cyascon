...
ASCON_API void
ascon_aead128a_encrypt(ascon_aead_ctx_t* ctx,
                      ...
                      size_t tag_len)
{
    ...
    ascon_aead128a_assoc_data_update(&temp_ctx, assoc_data, assoc_data_len);
    const size_t new_ct_bytes = ascon_aead128a_encrypt_update(&temp_ctx, ciphertext,
                                                              plaintext,
                                                              plaintext_len);
    ascon_aead128a_encrypt_final(&temp_ctx, ciphertext + new_ct_bytes,
                                 tag, tag_len);
}

ASCON_API bool
ascon_aead128a_decrypt(ascon_aead_ctx_t* ctx,
                       uint8_t* plaintext,
                       const uint8_t* assoc_data,
                       ...
                       size_t expected_tag_len)
{
    ...
    bool is_tag_valid;
    ascon_aead128a_assoc_data_update(&temp_ctx, assoc_data, assoc_data_len);
    const size_t new_pt_bytes = ascon_aead128a_decrypt_update(&temp_ctx,
                                                              plaintext,
                                                              ciphertext,
                                                              ciphertext_len);
    ascon_aead128a_decrypt_final(&temp_ctx, plaintext + new_pt_bytes,
...
ASCON_API size_t
ascon_aead128a_encrypt_intermediate(ascon_aead_ctx_t* const ctx,
                                    uint8_t* const ciphertext,
                                    uint8_t* tag,
                                    size_t tag_len)
{
    ...
    if (temp_ctx.bufstate.flow_state != ASCON_FLOW_AEAD128a_ENCRYPT_UPDATED)
    {
        // Finalise the associated data if not already done so.
        ascon_128a_finalise_assoc_data(ctx);
    }
    ...
    // If there is any remaining less-than-a-block plaintext to be absorbed
    // cached in the buffer, pad it and absorb it.
    if (temp_ctx.bufstate.buffer_len >= ASCON_RATE)
    {
        // Absorb plaintext in buffer
        temp_ctx.bufstate.sponge.x0 ^= bigendian_decode_u64(temp_ctx.bufstate.buffer);
        const uint_fast8_t second_half =
                (uint_fast8_t) (temp_ctx.bufstate.buffer_len - ASCON_RATE);
        temp_ctx.bufstate.sponge.x1 ^= bigendian_decode_varlen(
                temp_ctx.bufstate.buffer + ASCON_RATE, second_half);
        temp_ctx.bufstate.sponge.x1 ^= PADDING(second_half);
        // Squeeze out the ciphertext
        bigendian_encode_u64(ciphertext, temp_ctx.bufstate.sponge.x0);
        bigendian_encode_varlen(ciphertext + ASCON_RATE, temp_ctx.bufstate.sponge.x1,
                                second_half);
    }
    else
    {
        ...
        // Squeeze out the last ciphertext bytes, if any
        bigendian_encode_varlen(ciphertext, temp_ctx.bufstate.sponge.x0,
                                temp_ctx.bufstate.buffer_len);
    }
    freshly_generated_ciphertext_len += temp_ctx.bufstate.buffer_len;
    // End of encryption, start of tag generation.
    // Apply key twice more with a permutation to set the state for the tag.
    temp_ctx.bufstate.sponge.x2 ^= temp_ctx.k0;
    temp_ctx.bufstate.sponge.x3 ^= temp_ctx.k1;
...
ASCON_API size_t
ascon_aead128a_decrypt_intermediate(ascon_aead_ctx_t* const ctx,
                                    uint8_t* plaintext,
                                    uint8_t* tag,
                                    const size_t tag_len)
{
    ...
    if (temp_ctx.bufstate.flow_state != ASCON_FLOW_AEAD128a_DECRYPT_UPDATED)
    {
        // Finalise the associated data if not already done sos.
        ascon_128a_finalise_assoc_data(ctx);
    }
    ...
    {
        // Absorb ciphertext in buffer
        const uint64_t c_0 = bigendian_decode_u64(temp_ctx.bufstate.buffer);
        const uint_fast8_t second_half =
                (uint_fast8_t) (temp_ctx.bufstate.buffer_len - ASCON_RATE);
        const uint64_t c_1 = bigendian_decode_varlen(
                temp_ctx.bufstate.buffer + ASCON_RATE, second_half);
        // Squeeze out last plaintext bytes
        bigendian_encode_u64(plaintext, temp_ctx.bufstate.sponge.x0 ^ c_0);
        bigendian_encode_varlen(plaintext + ASCON_RATE,
                                temp_ctx.bufstate.sponge.x1 ^ c_1,
                                second_half);
        // Final state changes at decryption's end
        temp_ctx.bufstate.sponge.x0 = c_0;
        temp_ctx.bufstate.sponge.x1 &= ~mask_most_signif_bytes(second_half);
        temp_ctx.bufstate.sponge.x1 |= c_1;
        temp_ctx.bufstate.sponge.x1 ^= PADDING(second_half);
    }
    else
