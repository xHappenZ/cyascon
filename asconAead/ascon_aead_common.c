...
{
  ascon_aead_ctx_t* ctx = malloc(sizeof(ascon_aead_ctx_t));
  return ctx;
}
...
ASCON_API bool
ascon_aead_is_tag_valid_intermediate(const uint8_t* expected_tag,
                                        size_t expected_tag_len,
                                        const uint8_t actual_tag[expected_tag_len],
                                        size_t actual_tag_len)
{
    uint64_t expected_tag_chunk;
    uint64_t actual_tag_chunk;
    bool tags_differ = false;
    while (expected_tag_len > ASCON_AEAD_TAG_MIN_SECURE_LEN)
    {
        // Note: converting the expected tag from uint8_t[] to uint64_t
        ...
        // Constant time comparison expected vs computed digest chunk, part 1
        expected_tag_chunk = bigendian_decode_u64(expected_tag);
        actual_tag_chunk = bigendian_decode_u64(actual_tag);
        tags_differ |= (expected_tag_chunk ^ actual_tag_chunk);
        expected_tag += sizeof(expected_tag_chunk);
        actual_tag += sizeof(actual_tag_chunk);

        expected_tag_len -= sizeof(expected_tag_chunk);
        actual_tag_len -= sizeof(actual_tag_chunk);
        // Constant time comparison expected vs computed digest chunk, part 2
        expected_tag_chunk = bigendian_decode_u64(expected_tag);
        actual_tag_chunk = bigendian_decode_u64(actual_tag);
        tags_differ |= (expected_tag_chunk ^ actual_tag_chunk);
    ...
    // Extract the remaining n most significant bytes of expected/computed tags
    size_t remaining_expected = MIN(sizeof(expected_tag_chunk), expected_tag_len);
    size_t remaining_actual = MIN(sizeof(actual_tag_chunk), actual_tag_len);
    // uint64_t ms_mask = mask_most_signif_bytes((uint_fast8_t) remaining_expected);

    expected_tag_chunk = bigendian_decode_varlen(expected_tag, (uint_fast8_t) remaining_expected);
    actual_tag_chunk = bigendian_decode_varlen(actual_tag, (uint_fast8_t) remaining_actual);
    
    tags_differ |= (expected_tag_chunk ^ actual_tag_chunk);
    expected_tag += remaining_expected;
    actual_tag += remaining_actual;
    expected_tag_len -= remaining_expected;
    actual_tag_len -= remaining_actual;
    remaining_expected = MIN(sizeof(expected_tag_chunk), expected_tag_len);
    remaining_actual = MIN(sizeof(actual_tag_chunk), actual_tag_len);
    expected_tag_chunk = bigendian_decode_varlen(expected_tag, (uint_fast8_t) remaining_expected);
    actual_tag_chunk = bigendian_decode_varlen(actual_tag, (uint_fast8_t) remaining_actual);

    tags_differ |= (expected_tag_chunk ^ actual_tag_chunk) ^ (actual_tag_len ^ expected_tag_len);
    return !tags_differ; // True if they are equal
}
...
ASCON_API void
ascon_aead_free(ascon_aead_ctx_t* const ctx)
{
    free(ctx);
}
