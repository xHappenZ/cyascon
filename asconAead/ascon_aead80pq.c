...
ASCON_API void
ascon_aead80pq_encrypt(ascon_aead_ctx_t* ctx,
                      ...
                      size_t tag_len)
{
    ...
    ascon_aead80pq_assoc_data_update(&temp_ctx, assoc_data, assoc_data_len);
    const size_t new_ct_bytes = ascon_aead80pq_encrypt_update(&temp_ctx, ciphertext,
                                                              plaintext,
                                                              plaintext_len);
    ascon_aead80pq_encrypt_final(&temp_ctx, ciphertext + new_ct_bytes,
                                 tag, tag_len);
}

ASCON_API bool
ascon_aead80pq_decrypt(ascon_aead_ctx_t* ctx,
                       uint8_t* plaintext,
                       const uint8_t* assoc_data,
                       ...
                       size_t expected_tag_len)
{
    ...
    bool is_tag_valid;
    ascon_aead80pq_assoc_data_update(&temp_ctx, assoc_data, assoc_data_len);
    const size_t new_pt_bytes = ascon_aead80pq_decrypt_update(&temp_ctx,
                                                              plaintext,
                                                              ciphertext,
                                                              ciphertext_len);
    ascon_aead80pq_decrypt_final(&temp_ctx, plaintext + new_pt_bytes,
...
ASCON_API size_t
ascon_aead80pq_encrypt_intermediate(ascon_aead_ctx_t* const ctx,
                                    uint8_t* const ciphertext,
                                    uint8_t* tag,
                                    size_t tag_len)
{
    ...
    // End of encryption, start of tag generation.
    // Apply key twice more with a permutation to set the state for the tag.
    temp_ctx.bufstate.sponge.x1 ^= temp_ctx.k0 << 32U | temp_ctx.k1 >> 32U;
    temp_ctx.bufstate.sponge.x2 ^= temp_ctx.k1 << 32U | temp_ctx.k2 >> 32U;
    temp_ctx.bufstate.sponge.x3 ^= temp_ctx.k2 << 32U;
    ascon_permutation_12(&temp_ctx.bufstate.sponge);
    temp_ctx.bufstate.sponge.x3 ^= temp_ctx.k1;
    temp_ctx.bufstate.sponge.x4 ^= temp_ctx.k2;
    // Squeeze out tag into its buffer.
...
ASCON_API size_t
ascon_aead80pq_decrypt_intermediate(ascon_aead_ctx_t* const ctx,
                                    uint8_t* plaintext,
                                    uint8_t* tag,
